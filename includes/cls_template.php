...
//            }
    ...
    function smarty_prefilter_preCompile($source)
    {
        ...
        if ($file_type == '.dwt')
        {
            ...
            //处理动态页面的逻辑，
            //get_dyna_libs函数会根据当前模板（也就是index.dwt）查询template表，
            //得到动态页面数据。
            /* 检查有无动态库文件，如果有为其赋值 */
            ...
            if ($dyna_libs)
            {
                ...
                {
                    //我们可以看到匹配动态页面标签的表达式，其中的name由$dyna_libs中的键生成
                    $pattern = '/<!--\\s*TemplateBeginEditable\\sname="'. $region .'"\\s*-->(.*?)<!--\\s*TemplateEndEditable\\s*-->/s';

                    if (preg_match($pattern, $source, $reg_match))
                    {
                        //匹配到到模板中可编辑区域的引入标签如{include file=library/cat_goods.lbi}
                        $reg_content = $reg_match[1];
                        //这里会生成以$libs中的键，如/library/cat_goods.lbi为值的枚举数组
                        //这将被用来生成引入标签正则表达式
                        /* 生成匹配字串 */
                        ...
                        //生成引入标签正则表达式/{include\sfile=(library\/cat_goods.lbi)}/
                        $lib_pattern = '/{include\sfile=(' . substr($lib_pattern, 1) . ')}/';
                        /* 修改$reg_content中的内容 */
                        //将$libs存储的全局变量种，供接下来的回调函数dyna_libs_replace使用
                        $GLOBALS['libs'] = $libs;
                        //匹配$reg_content的内容，通过一个回调函数dyna_libs_replace修改其内容，也就是添加变量注册smarty标签
