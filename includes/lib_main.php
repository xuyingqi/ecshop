...
{
    //在模板表template取出动态页面
    //这里的$GLOBALS['_CFG']['template']大有学问，这里决定了采用哪套模板的template数据

    //其中字段number控制每次显示多少个值，
    //而字段type则控制属于哪个动态项,0为 固定项,1为分类下的商品，2为品牌下的商品，
    //3为文章列表,4为广告位.
    $sql = 'SELECT id, number, type FROM ' . $GLOBALS['ecs']->table('template') .
        " WHERE filename = '$tmp' AND type > 0 AND remarks ='' AND theme='"
        . $GLOBALS['_CFG']['template'] . "'";
    $res = $GLOBALS['db']->getAll($sql);

   foreach ($res AS $row)
    {
        //匹配类型，按type字段执行相应变量的注册
        switch ($row['type'])
        {
            case 1:
                /* 分类下的商品 */
                //给不同的模块赋值。比如在分类，就调用ssign_cat_goods()函数，
                //传递给模板参数’goods_cat_’ . $row[‘id’].
                $GLOBALS['smarty']->assign('goods_cat_' . $row['id'],
                    assign_cat_goods($row['id'], $row['number']));
            break;
            case 2:
                /* 品牌的商品 */
