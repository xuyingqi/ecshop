...
{
    //检查我们所传递的推荐类型是否在函数处理之内
    if (!in_array($type, array('best', 'new', 'hot')))
    {
        //如果传错了类型那么直接返回一个空数组那么前台就没有任何显示了
        return array();
    }

    //取不同推荐对应的商品
    //这里我们定义了一个静态数组，该数组在下边是用来存放不同推荐类型的商品所要
    static $type_goods = array();
    if (empty($type_goods[$type]))
    {
        //初始化数据
        //把$type_goods变为一个二维数组，下边就是我们的推荐类型标示，
        //这个数组在之后用来存储不同推荐类型的商品
        $type_goods['best'] = array();
        $type_goods['new'] = array();
        $type_goods['hot'] = array();
        //读取缓存文件，recommend_goods，着里边存放的是所有推荐的商品，
        //他的目录是temp/static_caches/recommend_goods.php
        // 这个缓存文件用处非常的大一定要打开看看里边内容的存储格式
        $data = read_static_cache('recommend_goods');
        //如果这个缓存文件中没有放着内容执行下边if以内的语句，
        //多数情况下这里是可以读到数据的，除非您在后台清楚了缓存，
        //这里我们先对如果得到了缓存数据进行分析，请看else以内的程序
        if ($data === false)
        {
        ...
        //如果我们从缓存文件中取出了数据，程序将会执行else以内的程序。
        else
        {
            //把从缓存文件中读到的数据放入$goods_data中进行存储
            $goods_data = $data;
        }
        //获得当前格林威治时间的时间戳
        $time = gmtime();
        //获取后台中 商店设置-显示设置-推荐商品排序 的配置信息
        $order_type = $GLOBALS['_CFG']['recommend_order'];

        //按推荐数量及排序取每一项推荐显示的商品 order_type可以根据后台设定进行各种条件显示
        //这里有定义了一个静态的数组$type_array
        //这个是在下边用来以每个推荐类型的标示作为键以推荐类型下所有的商品作为值
        // 来存储每个推荐类型的推荐商品。
        static $type_array = array();
        //这里定义了一个存放 所有推荐类型所对应的库文件 的数组变量，在下边用于数据循环所用
        $type2lib = array('best'=>'recommend_best', 'new'=>'recommend_new', 'hot'=>'recommend_hot');
        //因为 $type_array 在之前没有被用过所以这里一定为空，那么执行if以内的程序
        if (empty($type_array))
        {
            //这里对存放着 推荐类型以及各类型所对应着的库文件 的数组进行循环，
            //目的是取出这每个类型下的库文件需要显示的商品数量
            foreach($type2lib as $key => $data)
            {
                //这里因为$goods_data存放的是缓存中的数据所以有值，
                //这里需要看一下缓存文件，文件中是以推荐类型的标示
                // 如：best、new、hot 为键来存储数据的，
                //因为 $type2lib 数组的键是跟 $goods_data 中的键相对于的
                //所以这里用的 $type2lib 数组的键 $key 做为$goods_data键就取出了
                // 当前循环的 推荐下的所有商品了。
                //所有这一步会返回一个true，程序执行if以内的语句
        ...
        $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
                ...
                "promote_start_date, promote_end_date, g.goods_brief, g.goods_desc, g.goods_thumb, g.goods_img, RAND() AS rnd " .
                'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
...
function get_promote_goods($cats = '')
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
                "promote_start_date, promote_end_date, g.goods_brief, g.goods_desc, g.goods_thumb, goods_img, b.brand_name, " .
...
function assign_cat_goods($cat_id, $num = 0, $from = 'web', $order_rule = '')
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.market_price, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
               'g.promote_price, promote_start_date, promote_end_date, g.goods_brief, g.goods_desc, g.goods_thumb, g.goods_img ' .
            "FROM " . $GLOBALS['ecs']->table('goods') . ' AS g '.
            "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                    "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
