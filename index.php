...
if (!empty($_GET['gOo']))
{
    ...
    {
        /* 商品详情。*/
        $Loaction = 'activity.php?id=' . $_GET['goodsid'];
    }
...
{
    assign_template();
    /*首先它是调用函数assign_ur_here，这样得到了变量$position，
    在变量$position中有title和ur_here这两个变量。
    在index.dwt中会有如下代码： <title>{$page_title}<title>*/
    ...
    /* 页面中的动态内容 */
//    它的作用是注册动态页面的变量值
    //动态处理一些局部信息更新而不被缓存的情况
    assign_dynamic('index');
//    assign_dynamic('record');
}
//把网页显示出来
$smarty->display('index.dwt', $cache_id);
//$smarty->display('record.dwt', $cache_id);
